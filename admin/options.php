...
class NS_Plugin_Options {
	 ...
	 * Class constructor.
	 */
	public function __construct( $options = array(), $post_id = false ) {
		$this->shortname = 'wpig';
	...
	public function create_opening_tag( $value ) {
		...
		echo '<div class="suf-section fix ' . $className . '">' . "\n";
		if ( $group_class != '' ) {
	 ...
	 * @param $value
	 */
	public function create_suf_header_3( $value ) {
	...
	public function create_section_for_radio( $value ) {
		...
		foreach ( $value['options'] as $option_value => $option_text ) {
			...
			} elseif ( $this->get_option( $value['id'] ) === false && $value['std'] == $option_value ) {
				$checked = ' checked="checked" ';
			} else {
				$checked = ' ';
			}
